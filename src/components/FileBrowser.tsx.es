...
export default function FileBrowser({ files, onFilesChange }: FileBrowserProps) {
  ...
  // Mobile fallback picker: plain multi-file input without `webkitdirectory`
  // (Android/iOS browsers ignore webkitdirectory and never show a
  // "Use this folder" / "Select folder" button in their pickers).
  const mobileFolderFallbackRef = useRef<HTMLInputElement>(null);

  // Mobile browsers cannot pick folders via webkitdirectory, and
  // window.showDirectoryPicker is unsupported there too. Notably, Android
  // Chrome *claims* webkitdirectory support in feature detection, so we
  // must rely on UA sniffing as the only reliable signal.
  ...
  // Shared change handler for all file pickers. Resets input.value so that
  // re-selecting the exact same files/folder again still fires onChange.
  ...
  // "Upload Folder" button click: on desktop use the webkitdirectory input;
  // on mobile fall back to multi-file selection and recommend the ZIP route.
  const handleFolderButtonClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (isMobileBrowser) {
      setInfoMsg(
        "Your mobile browser can't select whole folders (no \"Use this folder\" button exists in mobile pickers). Best option: open your file manager app, long-press the folder, and tap Compress / Create ZIP — then upload that ZIP here; the folder structure will be preserved automatically. Otherwise, select multiple files now."
      );
      mobileFolderFallbackRef.current?.click();
    } else {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
      ...
      <div className="lg:col-span-5 flex flex-col h-[520px] border-r border-slate-800 pr-0 lg:pr-6">
        ...
        {infoMsg && (
          <div className="mb-3 flex items-start gap-2 p-2 bg-blue-950/40 border border-blue-900/60 text-blue-300 rounded text-xs animate-fadeIn">
            <Info className="w-4 h-4 flex-shrink-0 text-blue-400 mt-0.5" />
            <div className="flex-1 leading-relaxed">{infoMsg}</div>
            <button type="button" onClick={() => setInfoMsg(null)} className="text-blue-400 hover:text-blue-200">
              <X className="w-3.5 h-3.5" />
            </button>
          </div>
        )}
        ...
        >
          {/* Invisible pickers use sr-only instead of hidden (display:none),
              because mobile Safari refuses programmatic clicks on
          ...
          <input
            ...
            className="sr-only"
          />
          {/* Mobile fallback: folder picking is unsupported on mobile browsers,
              so offer plain multi-file selection (no accept filter) instead. */}
          <input
            type="file"
            ref={mobileFolderFallbackRef}
            onChange={handlePickerChange}
            multiple
            className="sr-only"
          />
          ...
          <div className="flex gap-2 mb-1.5">
            ...
            >
              {isMobileBrowser ? "Upload Multiple Files" : "Upload Folder"}
            </button>
          </div>
          <p className="text-[10px] text-slate-500 mt-1 leading-normal">
            Extracts ZIPs & preserves folder directory structure on upload.
            {isMobileBrowser && " On mobile, compress your folder to a ZIP first to keep its structure."}
          </p>
        </div>
